...
export function HeroSection() {
    return (<section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-14">
      {/* Background layers */}
      <div className="absolute inset-0 grid-bg opacity-60" aria-hidden="true"/>
      <div className="absolute top-1/4 -left-32 w-[600px] h-[600px] orb-primary blur-3xl" aria-hidden="true"/>
      <div className="absolute bottom-1/4 -right-32 w-[500px] h-[500px] orb-accent blur-3xl" aria-hidden="true"/>
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-5 py-28 w-full max-w-4xl mx-auto">
        ...
        <div className="w-full max-w-2xl animate-fade-up delay-300">
          <div className="flex items-center gap-2 rounded-2xl bg-surface border border-border p-2 shadow-card focus-within:border-primary/50 focus-within:shadow-glow-primary transition-all duration-200">
            <div className="pl-2 text-faint shrink-0" aria-hidden="true">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-4.35-4.35M11 19a8 8 0 100-16 8 8 0 000 16z"/>
              </svg>
            </div>
            <input type="text" readOnly placeholder="Paste a claim, headline, or tweet…" aria-label="Enter a claim to verify" className="flex-1 bg-transparent py-2.5 text-[0.9375rem] text-faint placeholder:text-faint cursor-pointer border-none outline-none focus:ring-0 rounded-none" onClick={() => { if (typeof window !== 'undefined')
        window.location.href = '/verify'; }}/>
            <a href="/verify" className="shrink-0 rounded-xl bg-primary px-5 py-2.5 text-sm text-white font-semibold hover:bg-primary/90 active:scale-[0.97] transition-all duration-100 flex items-center gap-2">
              Verify
        ...
        <dl className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-8 animate-fade-up delay-500">
          {STATS.map(({ value, label }) => (<div key={label} className="flex flex-col items-center gap-1">
