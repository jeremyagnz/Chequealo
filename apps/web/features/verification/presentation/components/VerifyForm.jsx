...
function LoadingDots() {
    return (<span className="inline-flex items-center gap-1" aria-label="Loading">
      {[0, 1, 2].map((i) => (<span key={i} className="h-1.5 w-1.5 rounded-full bg-primary-light animate-pulse-glow" style={{ animationDelay: `${i * 250}ms` }}/>))}
    </span>);
}
function ProcessingCard({ claim }) {
    const steps = [
        { label: 'Fetching live evidence', delay: 0 },
        { label: 'Running AI models', delay: 800 },
        { label: 'Calculating credibility', delay: 1600 },
    ];
    return (<div className="animate-fade-up rounded-2xl border border-border bg-surface p-7 shadow-card">
      ...
      <div className="space-y-2.5 pl-11">
        {steps.map(({ label, delay }, i) => (<div key={label} className="animate-slide-in-left flex items-center gap-2.5 text-[13px] text-muted" style={{ animationDelay: `${delay}ms` }}>
...
function ResultCard({ result }) {
    ...
    return (<div className="animate-fade-up rounded-2xl border border-border bg-surface shadow-card overflow-hidden">
      {/* Result header */}
      <div className="p-6 border-b border-border">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          {credibilityScore !== null && credibilityScore !== undefined && (<div className="shrink-0">
      ...
      {/* Evidence */}
      {Array.isArray(evidence) && evidence.length > 0 && (<div className="p-6 border-b border-border">
          ...
          <ol className="space-y-2">
            {evidence
                ...
                .map((ev, i) => {
                var _a;
                return (<li key={i} className="flex gap-3 text-[12.5px]">
                  <span className="shrink-0 text-faint tabular-nums mt-0.5">{i + 1}.</span>
                  <div className="min-w-0">
                    {ev.url ? (<a href={ev.url} target="_blank" rel="noopener noreferrer" className="text-accent-light hover:underline font-medium">
                        {(_a = ev.title) !== null && _a !== void 0 ? _a : ev.url}
                      </a>) : (<span className="text-muted font-medium">{ev.title}</span>)}
                    {ev.snippet && (<p className="text-faint mt-0.5 line-clamp-2">{ev.snippet}</p>)}
                  </div>
                </li>);
            })}
          </ol>
      ...
      {/* AI responses */}
      {Array.isArray(aiResponses) && aiResponses.length > 0 && (<div className="p-6">
          <h3 className="mb-3 text-[13px] font-semibold text-text flex items-center gap-2">
            <svg className="h-3.5 w-3.5 text-faint" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div className="space-y-3">
            {aiResponses
                .map((resp, i) => {
                var _a;
                return (<div key={i} className="rounded-xl bg-surface2 border border-border p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[12px] font-semibold text-muted capitalize">
                      {(_a = resp.provider) !== null && _a !== void 0 ? _a : `Model ${i + 1}`}
                    </span>
                    {resp.verdict && <Badge verdict={resp.verdict} size="sm"/>}
                  </div>
                  {resp.reasoning && (<p className="text-[12.5px] text-faint leading-relaxed">{resp.reasoning}</p>)}
                </div>);
            })}
          </div>
        </div>)}
...
export function VerifyForm() {
    ...
    function handleReset() {
        ...
        setPolling(false);
        pollCount.current = 0;
        setTimeout(() => { var _a; return (_a = textareaRef.current) === null || _a === void 0 ? void 0 : _a.focus(); }, 50);
    }
    ...
    return (<div className="space-y-5 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="rounded-2xl border border-border bg-surface shadow-card focus-within:border-primary/50 focus-within:shadow-glow-primary transition-all duration-200">
          <textarea ref={textareaRef} value={claim} onChange={(e) => setClaim(e.target.value)} placeholder="Paste a claim, news headline, tweet, or statement to verify…" aria-label="Enter claim to verify" rows={4} disabled={loading || polling} className="w-full resize-none rounded-t-2xl bg-transparent px-5 pt-5 pb-3 text-[0.9375rem] text-text placeholder:text-faint border-none outline-none focus:ring-0 disabled:opacity-60 disabled:cursor-not-allowed"/>
          ...
          <div className="flex items-center justify-between px-4 pb-3 pt-1">
            ...
            <button type="submit" disabled={!canSubmit} className="flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm text-white font-semibold hover:bg-primary/90 active:scale-[0.97] disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-100 shadow-glow-primary">
              {loading ? (<>
      ...
      {/* Error */}
      {error && (<div className="animate-fade-in flex items-start gap-3 rounded-xl border border-verdict-false/25 bg-verdict-false/10 px-4 py-3" role="alert">
      ...
      {/* Result */}
      {result && (<>
          {result.status === 'failed' ? (<div className="animate-fade-up rounded-2xl border border-verdict-false/25 bg-verdict-false/10 px-6 py-5" role="alert">
            ...
            </div>) : (<ResultCard result={result}/>)}

          <button type="button" onClick={handleReset} className="text-[13px] text-muted hover:text-text transition-colors flex items-center gap-1.5">
