...
const NAV_ITEMS = [
    {
        href: '/dashboard',
        label: 'Dashboard',
        icon: (<svg fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
...
function LogoIcon() {
    return (<svg fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
    </svg>);
}
export function Sidebar() {
    const pathname = usePathname();
    return (<aside className="flex h-full w-60 flex-col border-r border-border bg-surface" aria-label="Application navigation">
      {/* Logo */}
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5" aria-label="Primary">
        {NAV_ITEMS.map((item) => {
            const isActive = item.href === '/verify'
                ? pathname === item.href
                : pathname.startsWith(item.href);
            return (<Link key={item.href} href={item.href} className={`group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13.5px] font-medium transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 ${isActive
                    ? 'bg-primary/15 text-primary-light'
                    : 'text-muted hover:bg-surface2 hover:text-text'}`} aria-current={isActive ? 'page' : undefined}>
              {/* Active indicator */}
              {isActive && (<span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full bg-primary" aria-hidden="true"/>)}
              ...
              {'badge' in item && item.badge && (<span className="ml-auto text-[10px] font-semibold rounded-full bg-primary/20 text-primary-light px-1.5 py-0.5 border border-primary/25">
                  {item.badge}
                </span>)}
            </Link>);
        })}
