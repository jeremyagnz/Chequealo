    ...
    <form onSubmit={handleSave} className="space-y-6 max-w-lg">
      ...
      <section className="animate-fade-up rounded-2xl border border-border bg-surface p-6 shadow-card">
        ...
        {loading ? (
          ...
          <SelectField
            ...
            description="All providers produce equivalent results; some are faster or cheaper."
            value={form.preferredAIProvider ?? 'openai'}
            options={AI_PROVIDERS}
            onChange={(v) => setForm((f) => ({ ...f, preferredAIProvider: v }))}
          />
        )}
      </section>
      ...
      <section className="animate-fade-up delay-100 rounded-2xl border border-border bg-surface p-6 shadow-card">
        ...
        {loading ? (
          ...
          <SelectField
            id="search-provider"
            label="Preferred Search Provider"
            value={form.preferredSearchProvider ?? 'serper'}
            options={SEARCH_PROVIDERS}
            onChange={(v) => setForm((f) => ({ ...f, preferredSearchProvider: v }))}
          />
        )}
      </section>
      ...
      <div className="animate-fade-up delay-300 flex items-center gap-3">
        <button
          ...
          className="rounded-xl bg-primary px-6 py-2.5 text-sm text-white font-semibold hover:bg-primary/90 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-100 shadow-glow-primary"
        >
          {saving ? (
        ...
        {saved && (
          <span className="flex items-center gap-1.5 text-[13px] text-verdict-true animate-fade-in">
