...
type Verdict = 'TRUE' | 'FALSE' | 'MISLEADING' | 'UNVERIFIED' | 'pending' | 'processing' | 'failed' | string;

const CONFIG: Record<string, { bg: string; text: string; border: string; icon: ReactElement }> = {
  TRUE: {
  ...
  pending: {
    ...
    icon: (
      <svg className="h-3 w-3 animate-spin-slow" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden="true">
...
export function Badge({ verdict, size = 'md', className = '' }: BadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border font-semibold ${bg} ${text} ${border} ${sizeClasses} ${className}`}
    >
      {icon}
      {verdict.toUpperCase()}
    </span>
