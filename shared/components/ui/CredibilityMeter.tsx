...
function getScoreColor(score: number): string {
  if (score >= 75) return 'rgb(var(--color-true))';
  if (score >= 50) return 'rgb(var(--color-accent))';
  if (score >= 25) return 'rgb(var(--color-misleading))';
  return 'rgb(var(--color-false))';
}

function getScoreLabel(score: number): string {
  if (score >= 80) return 'Highly Credible';
  if (score >= 60) return 'Likely True';
...
export function CredibilityMeter({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`Credibility score: ${clampedScore} out of 100 — ${label}`}
    >
      <svg
        width={size}
        height={size}
